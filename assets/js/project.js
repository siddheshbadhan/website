AOS.init();
/* Project Cards */

const projectcards = document.querySelector(".projectcards");

// Array of object for projects
const projects = [
  {
    title: "Unified Payment Interface",
    cardImage: "assets/images/project-page/qr.jpeg",
    description: "Performed problem mapping, competitive analysis, and pricing analysis to shape the customer value proposition. Utilized market research to create a product vision, roadmap, and strategy aligned with market trends.",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Githublink: "https://image.flaticon.com/icons/png/512/643/643350.png",
  },
  {
    title: "Comprehensive Tech Strategy",
    cardImage: "assets/images/project-page/mercury.webp",
    description: "Proposed changes in the current infrastructure through A/B testing to boost user base and profits. Developed a 6-quarter product roadmap, aimed at launching a social media platform facilitating interaction between startup founders and investors.",
    tagimg: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Previewlink: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Githublink: "https://image.flaticon.com/icons/png/512/643/643350.png",
  },
  {
    title: "Deloitte Survey Analysis",
    cardImage: "assets/images/project-page/deloitte.jpg",
    description: "Identified binge-watching drivers through survey analysis. Developed predictive models using KNN and Logistic Regression to forecast user behavior and predict binge-watching likelihood on various devices within a no-ads subscription service.",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Githublink: "https://image.flaticon.com/icons/png/512/643/643350.png",
  },
  {
    title: "Customer-centric shopping solution",
    cardImage: "assets/images/project-page/hypeerconnectier.jpeg",
    description: "Developed a proximity marketing solution for retailers in malls and hyperlocal markets, including product differentiation, market research, and Bluetooth beacon prototypes for smart shopper recommendations.",
    tagimg: "https://miro.medium.com/max/2800/0*U2DmhXYumRyXH6X1.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Real-Time Weed Detection",
    cardImage: "assets/images/project-page/weed.webp",
    description: "Utilized CNN and ResNet-50 models to detect weeds in farms. Integrated point cloud data for 3D plant environment reconstruction and employed machine learning algorithms to effectively differentiate weeds from plants, achieving a 90% accuracy rate.",
    tagimg:
      "data:image/png;base64,",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Tableau for Placement Statistics",
    cardImage: "assets/images/project-page/tableau.jpeg",
    description: "Launched a live interactive Tableau dashboard having heatmaps, stories, and flowcharts on a website for gaining insights related to S.P.I.T. placements to enable students to tackle the placement process in an informed manner.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    Githublink: "",
  },
  {
    title: "Wearable for Dementia patients",
    cardImage: "assets/images/project-page/wearable.jpeg",
    description: "Designed a GPS-based device to monitor patients, guide them home, and alert caregivers in emergencies. Developed with ESP32 and LoRa technology for cost-effectiveness, simplicity, and extended battery life.",
    Previewlink: "",
    Githublink: "",
  },
];

// function for rendering project cards data
const showCards = () => {
  let output = "";
  projects.forEach(({ title, description, cardImage, Previewlink, Githublink }) => {
    output += `       
        <div class="column skill-card card" style="margin: 15px"data-aos="zoom-in-up" data-aos-easing="linear" data-aos-delay="300" data-aos-duration="600" >
          <div class="wrapper" style="background: url(${cardImage}) center / cover no-repeat;">
            <div class="header">
            </div>
            <div class="data">
              <div class="content">
              <div class="title-div">
                <h1 style="font-size: 0.8em" class="title"><a href="#">${title}></a></h1>
                <h5 class="title"><a href="#">${description}</a></h1>
                </div>
                
            <ul class="menu-content"><br>
                  <li><a href="${Previewlink}" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="25" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-monitor"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg></a></li>
                  <li><a href="${Githublink}" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="25" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-github"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg></a></li>
                </ul>
              </div>
            </div>
          </div>
        </div>`;
  });
  projectcards.innerHTML = output;
};
document.addEventListener("DOMContentLoaded", showCards);

function myFunction() {
  // Declare variables
  var input, button, i, skillcard, card, title;
  input = document.getElementById("myInput").value;
  input = input.toUpperCase();
  skillcard = document.getElementsByClassName("skill-card");
  card = document.getElementsByClassName("card");
  title = document.getElementsByClassName("title");

  // Loop through all list items, and hide those who don't match the search query
  for (i = 0; i < button.length; i++) {
    if (
      button[i].innerHTML.toUpperCase().includes(input) ||
      title[i].innerHTML.toUpperCase().includes(input)
    ) {
      skillcard[i].style.display = "";
      card[i].style.display = "";
    } else {
      skillcard[i].style.display = "none";
      card[i].style.display = "none";
    }
  }
}
